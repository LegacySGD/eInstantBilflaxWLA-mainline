...
public class BilflaxWLAEngineManagerTest
{
	...
	{
		logger = Logger.getLogger(BilflaxWLAEngineManagerTest.class);
		gameParams = XMLUtil.parseResource("paymodel/BilflaxWLAIW/GameParams.xml").getDocumentElement();
		previousOutcome = XMLUtil.parseResource("paymodel/BilflaxWLAIW/InitOutcome.xml").getDocumentElement();
		...
		throws Exception
	{
		...
		// MerchandisePrize
		playerInput = new PlayerInputBuilder().appendNonCashPrizeInput("01-01-01", "BMW X5 xDrive45e Plug-in Hybrid", 1, 50).build();
			...
			throws Exception
	{
		...
		playerInput = new PlayerInputBuilder().appendNonCashPrizeInput("01-01-01", "BMW X5 xDrive45e Plug-in Hybrid", 1, 100).build();
		outcome = OutcomeUtil.getSignedData(outcome, "Outcome");
		outcome = manager.play(InputUtil.buildInput(gameParams, XMLUtil.getElement(outcome), playerInput));
		logger.debug(outcome);
